...
def train():
    argp = Parser()
    argp.parser.add_argument('--quant_bit', default=None, type=int)
    argp.parser.add_argument('--train_mode', default=None)
    ...
    if os.path.exists(os.path.join(pretrained_path, f"model_iters_{iteration_id}.pth")):
        ...
        pth_files = glob.glob(os.path.join(pretrained_path, "*.pth"))
    ...
    print("===> Further trained basing on model path: ", arg_dict['pretrained'])
    ...
    # Build lr scheduler
    # cosine_lr = CosineRestartLr(arg_dict['lr'], [arg_dict['max_iters']], [1], 1e-5)
    cosine_lr = CosineRestartLr(arg_dict['lr'], arg_dict['period'], arg_dict['restart_factor'], arg_dict['min_lr'])
        ...
        with tqdm(total=print_freq) as bar:
            for feature, label, _ in dataset:        
                if arg_dict['cpu']:
                    ...
                    input, target = feature.cuda(non_blocking=True), label.cuda(non_blocking=True)
