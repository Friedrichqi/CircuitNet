...
def test(quant_bit: int = 8):
    for quant_part in [['encoder'], ['decoder'], ['encoder', 'decoder']]: 
        ...
        # ------------------------------------------------------------------
        if quant_part == ['encoder']:
            output_prefix = 'encoder'
        elif quant_part == ['decoder']:
            output_prefix = 'decoder'
        else:
            output_prefix = 'all'
        output_path = os.path.join("./work_dir/qat_weights", f"{output_prefix}_{arg_dict['quant_bit']}bits.txt")
