...
def test(quant_bit: int = 8):
    ...
    arg_dict = vars(arg)
    arg_dict['pretrained'] = './work_dir/congestion_gpdl/model_iters_200000.pth'
    arg_dict['quant_bit'] = quant_bit

    for (train_mode, quant_part) in [
        ('encoder_activations', ['encoder']),
        ('decoder_activations', ['decoder']),
        ('all_activations', ['encoder', 'decoder']),
    ]:
        ...
        output_path = os.path.join("./work_dir/ptq_all", f"{output_prefix}_{arg_dict['quant_bit']}bits.txt")
